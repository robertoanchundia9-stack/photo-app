...
// In-memory likes cache for fast reads
let likesMap = {};
...
io.on('connection', (socket) => {
    ...
    socket.on('user_join', (data) => {
        ...
        if (data.userId) {
            io.emit('user_status_change', { userId: data.userId, online: true });
        }
    });

    socket.on('get_online_users', () => {
        const onlineUsers = Array.from(activeSockets.values()).map(u => u.userId);
    ...
    socket.on('toggle_like', async (data) => {
        ...
        if (supabase) {
            const { data: existing } = await supabase.from('likes').select('likes').eq('media_id', data.id).single();
            if (existing) {
                await supabase.from('likes').update({ likes: existing.likes + 1 }).eq('media_id', data.id);
            } else {
                await supabase.from('likes').insert({ media_id: data.id, likes: 1 });
            }
        }
    });
    ...
    socket.on('delete_blog_post', async (data) => {
        ...
        if (supabase) {
            ...
            if (!post || post.user_id !== data.userId) return;
            if (post.media && post.media.name) {
                try { await cloudinary.uploader.destroy(post.media.name); } catch(e) {}
            }
    ...
    socket.on('disconnect', () => {
        ...
        if (userData && userData.userId) {
            io.emit('user_status_change', { userId: userData.userId, online: false });
        }
